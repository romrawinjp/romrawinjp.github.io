...
Here is the result after run detected and cropped face image.
...
<details>
<summary>face_connections</summary>
...
<details>
<summary>UV coordinate</summary>

    uv_coordinate = np.array([[0.499976992607117, 0.652534008026123],
...
1. Call texture fucntion

        import torch
        import torch.nn.functional as F
        from pytorch3d.renderer import Textures
2. Change texture image to FloatTensor 

        texture_image = torch.from_numpy(body_new_face[::-1, :, ::-1]/255).to(device)
        texture_image = texture_image.type(torch.float32)
        texture_image.type()

3. Change UV coordinate to LongTensor 

        face_uvs = torch.from_numpy(tri_template.simplices)
        face_uvs = face_uvs.type(torch.LongTensor).to(device)
        face_uvs.type()
4. Change UV coordinate vertices to FloatTensor
        verts_uvs = torch.from_numpy(uv_coordinate)
        verts_uvs = verts_uvs.type(torch.FloatTensor).to(device)
        verts_uvs.type()
5. Change face landmarks to FloatTensor

        torch_face_landmark = torch_face_landmark.to(device)
        torch_faces = torch_faces.type(torch.FloatTensor).to(device)
        torch_face_landmark.type()

Run texture function 

    texture = TexturesUV(faces_uvs = [face_uvs],
                        verts_uvs=[verts_uvs],
                        maps=[texture_image])
...
Code for renderer is here
    ...
    images = renderer(facelandmark_to_mesh)
